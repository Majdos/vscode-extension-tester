...
import { IEditor, IEditorGroup, IEditorTab, IEditorView, WebElement } from "extension-tester-page-objects";
...
export class EditorView extends AbstractElement implements IEditorView {
     ...
     */
    async getActions(groupIndex: number = 0): Promise<WebElement[]> {
        const group = await this.getEditorGroup(groupIndex);
     ...
     */
    async getAction(title: string, groupIndex: number = 0): Promise<WebElement | undefined> {
        const group = await this.getEditorGroup(groupIndex);
